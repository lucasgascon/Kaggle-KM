...
""" Implement kernel PCA to reduce dimensionality of features
"""
...
class kernelPCA():
    ...
    def fit(self, X):
        ...
        if self.n_components < 0:
            ...
            while ratio < 0.8:
                self.n_components += 1
                ratio = sum(self.lmbda[i] for i in range(
                    0, self.n_components))/sum(self.lmbda)
    ...
    def transform(self, x):
        ...
        if not self.plt:
            ...
            explained_var = [sum(self.lmbd[-i] for i in range(1, i+1)) /
                             sum(self.lmbd) for i in range(1, len(self.lmbd)+1)]
            plt.figure()
            plt.plot(np.arange(1, len(self.lmbd)+1), explained_var)
            plt.xlabel('Number of components')
            plt.ylabel('Explained variance')
            plt.title(
                'Explained variance vs Number of components taken in the PCA for kernel ', self.kernel.__name__)
            return K@self.alpha[:, :self.n_components]


""" Implement Kmeans to initialize the EM algorithm for GMM
"""
...
class Kmeans():
    ...
    def fit(self, X, already_stacked=False):
        ...
        centroids = stacked_descriptors[np.random.choice(
            n_descriptors, self.k, replace=False), :]
        ...
        for i in range(self.max_iter):
            ...
            for j in range(self.k):
                distances[:, j] = np.linalg.norm(
                    stacked_descriptors - centroids[j], axis=1)
            clusters = np.argmin(distances, axis=1)
            # Update the centroids
            for j in range(self.k):
                centroids[j] = np.mean(
                    stacked_descriptors[clusters == j, :], axis=0)
    ...
    def predict(self, X):
        ...
        for j in range(self.k):
            distances[:, j] = np.linalg.norm(
                stacked_descriptors - self.centroids[j], axis=1)
        clusters = np.argmin(distances, axis=1)
        return clusters


""" Implement Bag of Visual Words for image classification
"""
...
""" Implement Fisher Vector for image classification
"""
...
def EM_algorithm(local_descriptors_list, n_gaussians, n_iter=100, starting_lmbda=None, eps=0.01):
    ...
    for t in trange(n_iter):
        ...
        for gauss_ind in range(m):
            ...
            arg_exp = np.array([- (x[i, :]-mu_t[gauss_ind, :]).T @ inv_Sigma_t @
                               (x[i, :]-mu_t[gauss_ind, :]) / 2 for i in range(n)])
            log_tau_t[:, gauss_ind] = np.log(
                alpha_t[gauss_ind]) - 0.5 * sign * logdet - d * np.log(2*np.pi)/2 + arg_exp

        tau_t = np.exp(log_tau_t - logsumexp(log_tau_t,
                       axis=1).reshape(n, 1))  # normalize tau_t
        # STEP M
        sum_tau_t = np.sum(tau_t, axis=0)
        alpha_t = sum_tau_t/n
        for gauss_ind in range(m):
            mu_t[gauss_ind, :] = tau_t[:, gauss_ind].T@x/sum_tau_t[gauss_ind]
            Sigma_t[gauss_ind, :] = np.diag((tau_t[:, gauss_ind]*(x-mu_t[gauss_ind, :]).T@(
                x-mu_t[gauss_ind, :]))/sum_tau_t[gauss_ind] + eps * np.eye(d, d))
...
class FisherVectorGMM:
    ...
    def transform(self, local_descriptors):
        ...
        gmm = {'alpha': self.gmm.weights_, 'mu': self.gmm.means_,
        ...
        if self.handmade:
            ...
            for t in range(T):
                ...
                logsum = gmm['alpha'][1]*multivariate_normal_density(
                    x, gmm['mu'][1, :], gmm['sigma'][1, :])
                for i in range(1, n_gaussians):
                    logsum += np.log(1+np.exp(logsum-np.log(
                        gmm['alpha'][i] * multivariate_normal_density(x, gmm['mu'][i, :], gmm['sigma'][i, :]))))
            ...
            g_mu = (S1 - (gmm['mu'] * S0.reshape(-1, 1))) / \
                (np.sqrt(gmm['alpha']).reshape(-1, 1)*gmm['sigma'])
            g_sigma = (S2 - 2 * gmm['mu'] * S1 + (gmm['mu'] ** 2 - gmm['sigma']**2) *
                       S0.reshape(-1, 1))/(np.sqrt(2 * gmm['alpha']).reshape(-1, 1)*gmm['sigma']**2)
            fish_vect = np.concatenate(
                (g_alpha, g_mu.flatten(), g_sigma.flatten()))
            fish_vect = np.sign(fish_vect) * np.sqrt(np.abs(fish_vect))
            fish_vect /= np.sqrt(np.dot(fish_vect, fish_vect))
        else:
            fish_vect = fisher_vector(local_descriptors, self.gmm)
        return fish_vect
