...
""" Implementation of the Histogram of Oriented Gradients (HOG) algorithm
"""
...
def calculate_hog(image, local_descriptor=False, handmade=False):
    ...
    if handmade:
        ...
        histogram_points_nine = np.zeros(
            (32//cell_size, 32//cell_size, num_bins))
            ...
            for j in range(0, 32, 8):
                ...
                for k in range(cell_size):
                    for l in range(cell_size):
                        ...
                        Vj = magnitude_values[k, l] * np.abs(
                            hist_bins[id_bin_j] - angle_values[k, l]) / step

                        if id_bin_j == 8:
                            bins[id_bin_j] += Vj
                        else:
                            Vjplus1 = magnitude_values[k, l] * np.abs(
                                hist_bins[id_bin_j+1] - angle_values[k, l]) / step
                            bins[id_bin_j] += Vj
        ...
        feature_vectors = np.zeros(
            (32//cell_size - 1, 32//cell_size - 1, num_bins*4))
        ...
        if local_descriptor:
            ...
            feature_vectors = hog(image, pixels_per_cell=(8, 8), cells_per_block=(
                2, 2), channel_axis=-1, feature_vector=True, orientations=9)
...
""" Implementation of the Local Binary Pattern (LBP) algorithm
"""
...
""" Implementation of the Scale-Invariant Feature Transform (SIFT) algorithm
"""
...
def calculate_SIFT(image, sigma=1.6, num_octaves=4, num_scales=5, handmade=False):
    ...
    if handmade:
        ...
        dog = []
        for octave in range(num_octaves):
            octave_dogs = np.zeros(
                (num_scales-1, pyr_images[octave*num_scales].shape[0], pyr_images[octave*num_scales].shape[1]))
            for scale in range(num_scales-1):
                octave_dogs[scale, :, :] = pyr_images[octave +
                                                      scale] - pyr_images[octave+scale+1]
        ...
        if descriptors is None:
            print('No descriptors found for the image. Returning a zero vector.')
