...
"""This file is used to train a model and create a submission file. (Full pipeline)
    In all the feature extraction functions, both the handmade or pre-implemented version can be used.
"""
...
def main(args):
    ...
    """Set up submission name if not given
    """
    if args.subname == '0':
        if args.kernelSVM == 'gaussian_kernel' or args.kernelPCA == 'gaussian_kernel' or args.kernelSVM == 'laplacian_kernel' or args.kernelPCA == 'laplacian_kernel':
            if args.bow or args.fishervect:
                args.subname = args.kernelSVM + '_'+args.kernelPCA + '_C' + str(args.C) + '_sigma' + str(args.sigma) + '_PCA' + str(
                    args.PCA) + '_bow' + str(args.bow) + '_k' + str(args.k) + '_fishervect' + str(args.fishervect)
            else:
                args.subname = args.kernelSVM + '_'+args.kernelPCA + '_C' + str(args.C) + '_sigma' + str(
                    args.sigma) + '_PCA' + str(args.PCA) + '_hog' + str(args.hog) + '_raw' + str(args.raw) + '_lbp' + str(args.lbp)
        else:
            if args.bow or args.fishervect:
                args.subname = args.kernelSVM + '_'+args.kernelPCA + '_C' + str(args.C) + '_PCA' + str(
                    args.PCA) + '_bow' + str(args.bow) + '_k' + str(args.k) + '_fishervect' + str(args.fishervect)
            else:
                args.subname = args.kernelSVM + '_'+args.kernelPCA + '_C' + str(args.C) + '_PCA' + str(
                    args.PCA) + '_hog' + str(args.hog) + '_raw' + str(args.raw) + '_lbp' + str(args.lbp)

    """Create feature vectors for the training and test sets.
    """

    # Load the dataset
    # Assuming there's no header row
    trainset = pd.read_csv('data/Xtr.csv', header=None)
    ...
    if args.normalize:
        # For trainset
        train_images = np.array([row_to_image(row.values)
                                for index, row in trainset.iterrows()])
        # train_images = (train_images - np.mean(train_images, axis=0))/np.std(train_images, axis=0)

        # For testset
        test_images = np.array([row_to_image(row.values)
                               for index, row in testset.iterrows()])
        # test_images = (test_images - np.mean(test_images, axis=0))/np.std(test_images, axis=0)
    else:
        # For trainset
        train_images = np.array([row_to_image(row.values, normalize=False)
                                for index, row in trainset.iterrows()])
        # For testset
        test_images = np.array([row_to_image(row.values, normalize=False)
                               for index, row in testset.iterrows()])

    if args.augmented:
        new_train_images = np.array([np.fliplr(image)
    ...
    if args.kernelSVM == 'polynomial_kernel':
        kernels[args.kernelSVM] = lambda x, y: polynomial_kernel(
            x, y, params['p'])
    elif args.kernelSVM == 'gaussian_kernel':
        kernels[args.kernelSVM] = lambda x, y: gaussian_kernel(
            x, y, params['sigma'])
    elif args.kernelSVM == 'sigmoid_kernel':
        kernels[args.kernelSVM] = lambda x, y: sigmoid_kernel(
            x, y, params['gamma'], params['r'])
    elif args.kernelSVM == 'laplacian_kernel':
        kernels[args.kernelSVM] = lambda x, y: laplacian_kernel(
            x, y, params['sigma'])
    elif args.kernelSVM == 'chi2_kernel':
        kernels[args.kernelSVM] = lambda x, y: chi2_kernel(
            x, y, params['sigma'])

    if args.kernelPCA == 'polynomial_kernel':
        kernels[args.kernelPCA] = lambda x, y: polynomial_kernel(
            x, y, params['p'])
    elif args.kernelPCA == 'gaussian_kernel':
        kernels[args.kernelPCA] = lambda x, y: gaussian_kernel(
            x, y, params['sigma'])
    elif args.kernelPCA == 'sigmoid_kernel':
        kernels[args.kernelPCA] = lambda x, y: sigmoid_kernel(
            x, y, params['gamma'], params['r'])
    elif args.kernelPCA == 'laplacian_kernel':
        kernels[args.kernelPCA] = lambda x, y: laplacian_kernel(
            x, y, params['sigma'])
    elif args.kernelPCA == 'chi2_kernel':
        kernels[args.kernelPCA] = lambda x, y: chi2_kernel(
            x, y, params['sigma'])

    """Create global feature vectors by fitting on training data and transforming data
    """

    if args.bow:
        if args.hog:
            print("Calculating HOG features")
            list_train_features = [calculate_hog(
                image, local_descriptor=True) for image in tqdm(train_images)]
            list_test_features = [calculate_hog(
                image, local_descriptor=True) for image in tqdm(test_images)]
            args.hog = False
        elif args.sift:
            print("Calculating SIFT features")
            list_train_features = [calculate_SIFT(
                image) for image in tqdm(train_images)]
            list_test_features = [calculate_SIFT(
                image) for image in tqdm(test_images)]
            args.sift = False
        ...
        if train_vector is None:
            ...
            train_vector = np.concatenate(
                (train_vector, train_features), axis=1)
    ...
    if args.fishervect:
        ...
        train_features = np.array([fishvect.transform(
            trainfeatures) for trainfeatures in tqdm(list_train_features)])
        test_features = np.array([fishvect.transform(testfeatures)
                                 for testfeatures in tqdm(list_test_features)])
    ...
    """Create local feature vectors by concatenating them to the already existing feature vectors (to nothing if not)
    """
    ...
    if args.hog:
        ...
        train_hog_features = np.array(
            [calculate_hog(image) for image in tqdm(train_images)])
        test_hog_features = np.array(
            [calculate_hog(image) for image in tqdm(test_images)])
        ...
        if train_vector is None:
            ...
            train_vector = np.concatenate(
                (train_vector, train_hog_features), axis=1)
            test_vector = np.concatenate(
                (test_vector, test_hog_features), axis=1)
    ...
    if args.raw:
        if train_vector is None:
            ...
            train_vector = np.concatenate(
                (train_vector, train_images.reshape(-1, 3*32*32)), axis=1)
            test_vector = np.concatenate(
                (test_vector, test_images.reshape(-1, 3*32*32)), axis=1)

    if args.lbp:
        print("Calculating LocalBinaryPattern features")
        train_lbs_features = np.array(
            [calculate_LocalBinaryPattern(image) for image in tqdm(train_images)])
        test_lbs_features = np.array(
            [calculate_LocalBinaryPattern(image) for image in tqdm(test_images)])
        ...
        if train_vector is None:
            ...
            train_vector = np.concatenate(
                (train_vector, train_lbs_features), axis=1)
            test_vector = np.concatenate(
                (test_vector, test_lbs_features), axis=1)

    if args.sift:
        print("Calculating SIFT features")
        train_sift_features = np.array(
            [np.mean(calculate_SIFT(image), axis=0)for image in tqdm(train_images)])
        test_sift_features = np.array(
            [np.mean(calculate_SIFT(image), axis=0) for image in tqdm(test_images)])
        ...
        if train_vector is None:
            ...
            train_vector = np.concatenate(
                (train_vector, train_sift_features), axis=1)
            test_vector = np.concatenate(
                (test_vector, test_sift_features), axis=1)
    ...
    if not args.to_submit:
        X_train, X_val, y_train, y_val = train_test_split(
            train_vector, train_labels, test_size=0.1, random_state=29)
    else:
    ...
    if args.PCA != 0:
        kernel_pca = kernelPCA(
            kernels[args.kernelPCA], args.PCA, with_norm=args.with_norm)
    ...
    if args.strat == 'onevsall':
        ...
        for class_label in np.unique(train_labels):
            ...
            if args.SVM == 'SGD':
                ...
                model = BinarySVM(
                    kernel=kernels[args.kernelSVM], C=params['C'])
        ...
        if not args.to_submit:
            ...
            for features in X_val:
                if args.SVM == 'SGD':
                    class_scores = [model.predict(features)
                                    for model in models.values()]

                elif args.SVM == 'CVXOPT':
                    class_scores = [model._decision_function(
                        [features])[0] for model in models.values()]
                ...
                predicted_class = max(
                    models.keys(), key=lambda x: class_scores[x])
        ...
        for id, first_class_label in enumerate(np.sort(np.unique(train_labels))[:-1]):
            for second_class_label in np.sort(np.unique(train_labels))[id+1:]:
                print('Training model for classes',
                      first_class_label, 'and', second_class_label)

                # Select the data
                X_train_partial = X_train[np.where(
                    (y_train == first_class_label) | (y_train == second_class_label))]
                y_train_partial = y_train[np.where(
                    (y_train == first_class_label) | (y_train == second_class_label))]

                # Create a binary label vector for the current class and the other class
                binary_labels = np.where(
                    y_train_partial == first_class_label, 1, -1)
                ...
                models[str(first_class_label)+"_" +
                       str(second_class_label)] = model
        if not args.to_submit:
            predictions = []
            for features in X_val:
                ...
                for id, first_class_label in enumerate(np.sort(np.unique(train_labels))[:-1]):
                    for second_class_label in np.sort(np.unique(train_labels))[id+1:]:
                        ...
                        if args.SVM == 'SGD':
                            pred = models[str(
                                first_class_label)+"_"+str(second_class_label)].predict(features)
                        elif args.SVM == 'CVXOPT':
                            pred = models[str(
                                first_class_label)+"_"+str(second_class_label)]._decision_function([features])[0]
                ...
                # Select the class label with the majority of votes
        ...
        for features in tqdm(test_vector):
            ...
            for id, first_class_label in enumerate(np.sort(np.unique(train_labels))[:-1]):
                for second_class_label in np.sort(np.unique(train_labels))[id+1:]:
                    ...
                    if args.SVM == 'SGD':
                        pred = models[str(first_class_label)+"_" +
                                      str(second_class_label)].predict(features)
        ...
        if args.startmode:
            ...
            submission_filepath = os.path.join(
                "submissions", submission_filename)
            test_predictions_df.to_csv(submission_filepath, index_label='Id')
            print("Submission saved at:", submission_filepath)
    if not args.to_submit:
        return accuracy
...
def parser_args(parser):
    ...
    parser.add_argument('--raw', action='store_true',
                        help='Use raw pixel features')
    parser.add_argument('--lbp', action='store_true',
                        help='Use Local Binary Pattern features')
    parser.add_argument('--PCA', type=int, default=0,
                        help='Number of components of PCA, 0 if not used and -1 if above 80 perc of variance is used')
    parser.add_argument('--kernelPCA', type=str,
                        default='linear_kernel', help='Kernel for PCA')
    parser.add_argument('--strat', type=str, default='onevsone',
                        help='Use One vs One or One vs All approach')
    parser.add_argument('--SVM', type=str, default='SGD',
                        help='SVM algorithm to use')
    parser.add_argument('--kernelSVM', type=str,
                        default='linear_kernel', help='Kernel for SVM')
    ...
    parser.add_argument('--epochs', type=int, default=1000,
                        help='Number of epochs for SGD')
    parser.add_argument('--subname', type=str, default='0',
                        help='Name of the submission file')
    parser.add_argument('--gamma', type=float, default=1.,
                        help='Gamma for sigmoid kernel')
    parser.add_argument('--r', type=float, default=1.,
                        help='r for sigmoid kernel')
    parser.add_argument('--to_submit', action='store_true',
                        help='Choose whether to train on the whole dataset or not')
    parser.add_argument('--bow', action='store_true',
                        help='Use Bag of Words features')
    parser.add_argument('--k', type=int, default=10,
                        help='Number of clusters for Bag of Words or of GMM components for Fisher Vector')
    parser.add_argument('--sift', action='store_true',
                        help='Use SIFT features')
    parser.add_argument('--fishervect', action='store_true',
                        help='Use fisher vector features')
    parser.add_argument('--with_norm', action='store_true',
                        help='Normalize gram matrix in PCA')
    parser.add_argument('--normalize', action='store_true',
                        help='Normalize the images')
    parser.add_argument('--augmented', action='store_true',
                        help='Augment data by flipping images')
    parser.add_argument('--startmode', action='store_true',
